...
<script>
    $(function () {
        ...
        $("input[name='ctype']").on("change", function () {
            // 请求的url
            var $url = "${ctx}/system/module/parentModule.do";
            // 请求携带的数据
            var $data = {"ctype": $(this).val()};
            // 发送ajax请求
                ...
                <div class="row data-type" style="margin: 0px">
                        ...
                        <select class="form-control"
                            ...
                            <c:forEach items="${menus}" var="item">
                                <c:if test="${item.ctype != 2}">
                                    <option ${module.parentId == item.id ?'selected':''}
                    ...
                    <div class="col-md-4 data">
                        <div class="form-group form-inline">
                            <div class="radio"><label><input type="radio" ${module.ctype==0?'checked':''} name="ctype"
                                                             value="0">主菜单</label></div>
                            <div class="radio"><label><input type="radio" ${module.ctype==1?'checked':''} name="ctype"
                                                             value="1">二级菜单</label></div>
                            <div class="radio"><label><input type="radio" ${module.ctype==2?'checked':''} name="ctype"
                                                             value="2">按钮</label></div>
                        </div>
                    ...
                    <div class="col-md-4 data">
                        <div class="form-group form-inline">
                            <div class="radio"><label><input type="radio" ${module.state==0?'checked':''} name="state"
                                                             value="0">停用</label></div>
                            <div class="radio"><label><input type="radio" ${module.state==1?'checked':''} name="state"
                    ...
                    <div class="col-md-4 data">
                        <div class="form-group form-inline">
                            <div class="radio"><label><input type="radio" ${module.belong==0?'checked':''} name="belong"
                                                             value="0">sass内部菜单</label></div>
                            <div class="radio"><label><input type="radio" ${module.belong==1?'checked':''} name="belong"
                                                             value="1">租用企业菜单</label></div>
                        </div>
                    </div>
